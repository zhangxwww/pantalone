...
async def get_lpr_data(db, dates):
    ...
    if len(not_found_dates) > 0:
        ...
        add_to_db_data = [
            schemas.LPRDataCreate(date=d["date"], lpr=d["rate"]) for d in aligned if d["date"] in not_found_dates
        ]
...
async def get_fund_holding_data(db, symbols):
    ...
    holdings = {}
    for symbol in symbols:
        holdings[symbol] = {"stock": stock_holdings[symbol], "bond": bond_holdings[symbol]}
...
async def get_stock_bond_info(db, stocks, bonds):
    ...
    def _crawl_stock(stock):
        ...
        return stock, {
            "code": stock,
            "abbreviation": data.get("股票简称", ""),
            "name": data.get("公司名称", ""),
            "industry": data.get("所属行业", ""),
            "market": data.get("所在市场", ""),
        }
    ...
    return {"stock": stock_info, "bond": bond_info}
